...
const [shippingOptionId, setShippingOptionId] = useState<string>(currentShippingOption);
  ...
  const submitShippingOption = async (soId: string, label: string, price: number, firstShipId?: string) => {
    ...
    if (!firstShipId) {
      // Handle the case where firstShipId is undefined
      console.error("First shipping option ID is undefined");
      return;
    }
  ...
  console.log("shipping options[0].id ",shipping_options?.[0]?.id)

  // const fetchShippingOptions = async () => {
  //   try {
  //     console.log("inside fetchShippingOptions() respose");
  ...
  //     setShippingOptionsFromApi(shipping_options);
  ...
  //   }
  // };
  // setFirstShippingOptionId(shipping_options?.[0]?.id)
...
useEffect(() => {
  ...
  onClick={() => {
    if (selectedShippingOption) {
      ...
      const firstShipId = shipping_options?.[0]?.id
      console.log('firstShipId', firstShipId)
      submitShippingOption(shippingOptionId, label, price, firstShipId);
    }
    else{
