...
const ProfileBillingAddress: React.FC<MyInformationProps> = ({ customer }) => {
  ...
  const currentInfo = useMemo(() => {
    ...
    const country =
      ...
      // // Use useEffect to log regionOptions when it changes
      // useEffect(() => {
...
{errors.billing_address?.message && (
  ...
  touched={touchedFields}
>
  {/* Find the country label by countryCode or use the countryCode if not found */}
  {customer.billing_address?.country_code ? (
    <option value={customer.billing_address.country_code}>
      {regionOptions.find(option => option.value === customer.billing_address?.country_code)?.label || customer.billing_address.country_code}
    </option>
  ) : (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div className="flex items-end justify-between" style={{ display: "flex", alignItems: "end", justifyContent: "flex-end" }}>
              <Button
                variant="secondary"
                onClick={() => {
                  console.log('customer.billing_address?.country_code', customer.billing_address?.country_code)
                  ...
                  setValue("billing_address.first_name", "", { shouldTouch: true });
  setValue("billing_address.last_name", "", { shouldTouch: true });
  setValue("billing_address.address_1", "", { shouldTouch: true });
  setValue("billing_address.address_2", "", { shouldTouch: true });
  setValue("billing_address.postal_code", "", { shouldTouch: true });
  setValue("billing_address.city", "", { shouldTouch: true });
  setValue("billing_address.province", "", { shouldTouch: true });
  setValue("billing_address.country_code",  undefined);
