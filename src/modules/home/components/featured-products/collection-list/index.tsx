  ...
  return (
    <div className="hide-scrollbar mb-5" style={{ fontFamily: 'Avenir Next LT W02 Regular', display: 'flex', overflowX: 'auto', paddingLeft: '160px' }}>
      {products.map((product, index) => {
        ...
        if (index === 1) delay = 4000; // 6 seconds delay for product[1]
        else if (index === 2) delay = 8000; // 12 seconds delay for product[2]
        else if (index === 3) delay = 4000;
...
const ProductItem: React.FC<ProductItemProps> = ({ thumbnail, title, subtitle, delayVisibility }) => {
  ...
  return (
    <div ref={imgRef} style={{ display: 'flex', flexBasis: 'calc(45% - 5px)', alignItems: 'center', marginRight: '5px', flexShrink: 0, opacity: isVisible ? 1 : 0, transition: 'opacity 1s' }}>
      ...
      {showText && ( // Only render the text when showText is true
        <div>
          <div style={{ display: "flex" }}>
            ...
            <div>
              <h3 style={{ fontSize: "20px", whiteSpace: 'normal', width: "", marginTop:"-5%",textAlign:"left" }}>{title}</h3>
              {subtitle && <p style={{ whiteSpace: 'normal', width: "80%", textAlign: "left" }}>{subtitle}</p>}
            </div>
          </div>
        </div>
      )}
